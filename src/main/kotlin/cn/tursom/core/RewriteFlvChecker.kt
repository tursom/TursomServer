...
class RewriteFlvChecker {
  ...
  private fun handleFrame(inputStream: InputStream, outputStream: OutputStream, lastTimestampRead: IntArray, lastTimestampWrite: IntArray): Boolean {
    // 读取前一个tag size
    val frame = pool.get()
   ...
   * 处理音/视频时间戳
   */
  private fun dealTimestamp(buffer: ByteBuffer, timestamp: Int, tagType: Int, lastTimestampRead: IntArray, lastTimestampWrite: IntArray) {
    // 如果是首帧
